...
- prior manual findings and known weak spots

Treat previous manual findings as covered. Probe the same product risk through a different user goal rather than repeating a request verbatim. When evidence is incomplete, choose prompts that expose uncertainty instead of assuming unavailable data or capabilities.
...
- does not ask for a field merely because it sounds plausible for the page domain
...
- differs materially from prior manual coverage

Avoid all-happy-path suites, invented capabilities, and overly strict expected text. Create challenge coverage with ambiguity, correction, source selection, map state, and continuation before reaching for unsupported capabilities. Prefer an explicit clarification path when source choice or user intent materially changes the result.
